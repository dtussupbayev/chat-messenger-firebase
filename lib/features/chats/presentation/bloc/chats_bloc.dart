...
class ChatsBloc extends Bloc<ChatsEvent, ChatsState> {
  ...
  Future<void> _onLoadChats(LoadChats event, Emitter<ChatsState> emit) async {
    ...
    try {
      ...
      final chatRoomsStream = await _getChatRoomsUseCase.execute(const NoParams());

      emit(state.copyWith(isLoading: false, uid: uid, chatRoomsStream: chatRoomsStream));
    } catch (e) {
      emit(state.copyWith(isLoading: false, error: e.toString()));
    }
  }

  Future<void> _onGetUserInfo(GetUserInfo event, Emitter<ChatsState> emit) async {
    try {
      final id = event.chatRoomId.replaceFirst(event.myUid, '').replaceFirst('_', '');

      final user = await _getUserInfoUseCase.execute(GetUserInfoParams(uid: id));
